...
common::Error Gpio::setMode(const Mode mode) {
    ...
    esp_err_t espErrorCode = gpio_set_direction(static_cast<gpio_num_t>(pin_), static_cast<gpio_mode_t>(mode));
...
common::Error Gpio::setLevel(const Level level) {
    ...
    esp_err_t espErrorCode = gpio_set_level(static_cast<gpio_num_t>(pin_), static_cast<uint32_t>(level));
    if(espErrorCode != ESP_OK) {
        return common::Error::FAIL;
    }
...
common::Error Gpio::configurePullUpDown(const bool pullUpEnable, const bool pullDownEnable) {
    if(pin_ == Pin::PIN_NOT_ASSIGN) {
        return common::Error::INVALID_STATE;
    }
    ...
    esp_err_t espErrorCode = gpio_set_pull_mode(static_cast<gpio_num_t>(pin_), pullMode);
    if(espErrorCode != ESP_OK) {
        return common::Error::FAIL;
    }
...
common::Error Gpio::setInterrupt(const InterruptType interruptType, common::Callback cb, common::CallbackData data) {
    ...
    esp_err_t espErrorCode = gpio_set_intr_type(static_cast<gpio_num_t>(pin_), static_cast<gpio_int_type_t>(interruptType));
    if(espErrorCode != ESP_OK) {
        return common::Error::FAIL;
    }
