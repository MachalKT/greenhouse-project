...
common::Error HrTimer::init() {
    ...
    setTimerNumber();
    name_ = "Timer" + std::to_string(static_cast<int>(timerNumber_));
    ...
    esp_err_t errorCode = esp_timer_create(&timerArgs, &handle_);
    if(errorCode == ESP_ERR_INVALID_ARG) {
        ESP_LOGE("TIMER create", "ESP_ERR_INVALID_ARG");
        return common::Error::FAIL;
    }
    if(errorCode != ESP_OK) {
        return common::Error::FAIL;
...
common::Error HrTimer::startOnce(const uint32_t timeUs) {
...
common::Error HrTimer::startPeriodic(const uint32_t timeUs) {
