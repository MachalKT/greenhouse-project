...
common::Error I2c::write(const uint8_t deviceAddress, const uint8_t command) {
    ...
    errorCode = i2c_master_write_byte(cmdHandle, (deviceAddress << 1) | I2C_MASTER_WRITE, true);
    if(errorCode != ESP_OK) {
        return common::Error::FAIL;
    }
    ...
    errorCode = i2c_master_cmd_begin(I2C_PORT, cmdHandle, 1000 / portTICK_PERIOD_MS);
...
common::Error I2c::write(const uint8_t deviceAddress, const uint8_t registerAddress, const uint8_t* buffer, const size_t bufferLength) {
    i2c_cmd_handle_t cmdHandle = i2c_cmd_link_create();
...
common::Error I2c::read(const uint8_t deviceAddress, uint8_t* buffer, const size_t bufferLength) {
    i2c_cmd_handle_t cmdHandle = i2c_cmd_link_create();
    ...
    errorCode = i2c_master_write_byte(cmdHandle, (deviceAddress << 1) | I2C_MASTER_READ, true);
    if(errorCode != ESP_OK) {
        return common::Error::FAIL;
    }

    errorCode = i2c_master_read(cmdHandle, buffer, bufferLength, I2C_MASTER_LAST_NACK);
    if(errorCode != ESP_OK) {
        return common::Error::FAIL;
    }
